...
const ALIAS_TO_CATEGORY = [
  ...
  { alias: "pickleball", category: "Pickleball" },
  { alias: "pickle ball", category: "Pickleball" },
  { alias: "tennis", category: "Tennis" },
...
export function resolveCanonicalCategory(input: string) {
  const normalized = normalizeCategoryInput(input);
  if (!normalized) return null;

  const direct = CANONICAL_CATEGORIES.find((category) => category.toLowerCase() === normalized);
...
export function suggestCanonicalCategories(input: string) {
  const normalized = normalizeCategoryInput(input);
  if (!normalized) return [...CANONICAL_CATEGORIES];

  const starts = CANONICAL_CATEGORIES.filter((category) => category.toLowerCase().startsWith(normalized));
  const aliases = ALIAS_TO_CATEGORY.filter(({ alias }) => alias.includes(normalized)).map(({ category }) => category);
  return Array.from(new Set([...starts, ...aliases])).slice(0, 6);
}
