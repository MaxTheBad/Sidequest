...
export default function SettingsPage() {
  ...
  async function saveProfile(e: FormEvent) {
    ...
    const changedWithoutUsername = changedFields.filter((f) => f !== "username");
    if (usernameCooldownActive) {
      setStatus(`You can only change your username once every 24 hours.${changedWithoutUsername.length ? ` Other changes saved: ${changedWithoutUsername.join(", ")}.` : ""}`);
    } else {
